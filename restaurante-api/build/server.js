...
async function cuisineRoutes(app2) {
  app2.get("/", listCuisines);
  app2.get("/:id", getCuisineById);
  app2.post("/", createCuisine);
  app2.put("/:id", updateCuisine);
  app2.delete("/:id", deleteCuisine);
}
...
async function setupApp() {
  ...
  await app.register((0, import_fastify_plugin.default)(async (fastify, opts) => {
    fastify.register(import_jwt.default, {
      secret: env.JWT_SECRET
      // Usa o segredo validado pelo Zod
    });
  }));
...
async function start() {
  try {
    ...
    console.log(`[SERVER] \u{1F680} Server is running on http://localhost:${env.PORT}`);
  } catch (err) {
